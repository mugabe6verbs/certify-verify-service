...
function serverOrigin(req) { const xfProto = (req.headers['x-forwarded-proto'] || '').toString().split(',')[0] || 'https'; return `${xfProto}://${req.headers.host}` }
function normalizeCountryCode(input) { if (!input) return null; const cc = String(input).trim().toUpperCase(); return /^[A-Z]{2}$/.test(cc) ? cc : null }
function addInterval(ms, interval) { const d = new Date(ms); if (interval === 'year') d.setUTCFullYear(d.getUTCFullYear() + 1); else d.setUTCMonth(d.getUTCMonth() + 1); return d.getTime() }
const AMOUNT_BY_PLAN = { pro_monthly: 19, pro_yearly: 190 }
const INTERVAL_BY_PLAN = { pro_monthly: 'month', pro_yearly: 'year' }
function amountToPlanId(amount) { const a = Number(amount); if (a === 19) return 'pro_monthly'; if (a === 190) return 'pro_yearly'; return null }
...
function isInvalidKeyErr(e) { const code = e?.response?.data?.error?.code || ''; return String(code).toLowerCase().includes('invalid_consumer_key_or_secret_provided') }
...
async function pesaToken(preferred = PESA_MODE) {
  ...
  let lastErr = null
  for (const b of order) { try { return await tokenFor(b) } catch (e) { if (isInvalidKeyErr(e)) { lastErr = e; continue } else { throw e } } }
  throw lastErr
}
