...
  ALLOW_MANUAL_PRO,
  ADMIN_TOKEN, // CRITICAL: Used for secure admin API access
  PESA_CONSUMER_KEY,
  PESA_CONSUMER_SECRET,
  PESA_BASE = 'demo', 
  PESA_IPN_ID,
  VERIFY_CNAME_TARGET = 'custom.getcertifyhq.com',
  // ADMIN_PASSWORD is REMOVED from the system, it's a security risk
} = process.env
...
// Admin Verification limit (less permissive now that it's just a check)
const adminVerifyLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 30,                  // 30 attempts per window (less sensitive without password brute force risk)
...
// CRITICAL FIX: Checks for the secure ADMIN_TOKEN environment variable
...
  if (!requireAdminToken(req, res)) return // Protected by the secure token
  const routes = []
...
// CRITICAL FIX: This uses the secure ADMIN_TOKEN (env var) for rescue operations.
function requireSecureAdminToken(req, res) {
  if (!ADMIN_TOKEN) { res.status(500).json({ ok:false, error:'Server missing ADMIN_TOKEN environment variable.' }); return false }
  const token = req.headers['x-admin-token'] || req.headers['X-Admin-Token']
  if (!token || token !== ADMIN_TOKEN) { res.status(403).json({ ok:false, error:'Forbidden (Admin Token required)' }); return false }
  return true
}
app.post('/admin/setPro', (req, res) => {
  if (!requireSecureAdminToken(req, res)) return // Use the secure check
  ;(async() => {
    try {
      if (!db) return res.status(500).json({ ok:false, error:'Server missing Firebase credentials' })
...
/* ============== Pesapal: token test (used for health check) ============== */
...
/* ============== Admin Verification (CRITICAL SECURITY FIX) ============== */
// This route is now used only to force a token refresh and check for an existing claim.
app.post('/api/admin/verify', adminVerifyLimiter, authenticate, async (req, res) => {
  try {
    // 1. User is already authenticated by the 'authenticate' middleware.
    const uid = req.user.uid;

    // 2. Force refresh the token to ensure custom claims are up-to-date.
    // This ensures RequireAdmin.jsx gets the latest claims.
    const idTokenResult = await admin.auth().getUser(uid); // Simple user fetch is enough to trigger a refresh

    // 3. Check for the admin claim. This should have been set via a rescue script.
    const isAdmin = req.user.admin === true;

    if (!isAdmin) {
      // This means a non-admin user is attempting to verify their credentials.
      return res.status(403).json({ ok: false, error: 'User is authenticated but lacks admin claims.' });
    }

    return res.json({ ok: true, uid, isAdmin: true });
  } catch (err) {
    console.error('ADMIN VERIFY ERROR:', err);
    return res.status(500).json({ ok: false, error: err?.message || 'Server error during verification check' });
  }
});
